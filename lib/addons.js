...
Addons.prototype = {
  addAddon: function* (app, addon, attachments, confirm) {
    let createAddon = require('heroku-cli-addons').createAddon;
    ...
    addon = yield co(createAddon(this.heroku, app.name, addon.plan.name, confirm, true, {as: attachment.name})).catch(catchShutdown);
  ...
  copyAddons: function* (oldApp, newApp, skipPG, confirm) {
    let data = yield {
    ...
    for (let addon of data.addons) {
      ...
      if (addon.app.name !== oldApp.name) {
        ...
        let toDB = yield this.addAddon(newApp, addon, attachments, confirm);
        ...
        try {
          yield this.addAddon(newApp, addon, attachments, confirm);
        } catch(err) {
  ...
  copyConfigVars: function* (oldApp, newApp, skipPG, excludeConfigsCsv) {
    console.log(`Copying config vars:`);
    let excludeKeys = (excludeConfigsCsv || '').split(',');
    ...
    let configVars = _.reduce(keysToKeep, function (result, key) {
      ...
      if (excludeKeys.indexOf(key) !== -1) return result;
      if (key.startsWith('HEROKU_')) return result;
